...
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(YAcceleratorCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(YAcceleratorCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						YAcceleratorCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/yacceleratorcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
